...
def ingest_book(client: weaviate.WeaviateClient, collection_name: str, file_path: str):
    ...
    # Create chunks using BookChunker with semantic strategy
    vectors_file = "semantic_vectors.pkl"
    if not os.path.exists(vectors_file):
        logger.info(f"Could not find cached chunks file: {vectors_file}")
    else:
        logger.info(f"Loaded chunks from {vectors_file}")
    ...
    with collection.batch.dynamic() as batch:
        for chunk in chunks:
            chunk_id = f"{chunk.chapter_num}_{chunk.chunk_index}"
            properties = {
            ...
            # Use cached or newly created vector
            vector = chunk.vector
            batch.add_object(
